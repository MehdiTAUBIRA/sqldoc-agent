          ...
          <div id="trigger-description" class="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
              <div class="flex justify-between items-center">
                ...
                <!-- ✅ Afficher le bouton ou un message selon les permissions -->
                <button 
                  v-if="canEdit"
...
const canEdit = computed(() => {
  return props.triggerDetails.can_edit || props.permissions.can_edit || false
})

const isOwner = computed(() => {
  return props.triggerDetails.is_owner || props.permissions.is_owner || false
})

const accessLevel = computed(() => {
  return props.triggerDetails.access_level || props.permissions.access_level || 'read'
})
...
onMounted(() => {
  ...
  console.log('🔍 [TRIGGER] Permissions:', props.permissions)
  console.log('🔍 [TRIGGER] Can Edit:', canEdit.value)
...
const saveDescription = async () => {
  if (!canEdit.value) {
    warning('🚫 Vous n\'avez pas les permissions pour modifier ce trigger')
    return
  }
  ...
  try {
    ...
    const response = await axios.post(`/trigger/${encodeURIComponent(props.triggerName)}/description`, { 
      description: form.value.description
    })
    
    if (response.data.success) {
      success('✅ Trigger description saved successfully')
    } else {
